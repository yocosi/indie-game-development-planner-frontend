...
<template>
  <div class="flex justify-center items-center flex-col mt-20">
    ...
    <form @submit.prevent="resetPassword" class="max-w-xl mx-auto border-2 border-gray-400 rounded-lg m-10 px-20 py-10 w-full">
      <div class="mb-10">
        <label for="email" class="block mb-2 text-lg font-medium text-gray-400 dark:text-white">E-mail address</label>
        <input v-model="email" type="text" id="email" class="bg-gray-900 border border-gray-400 text-gray-400 text-md rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-96 p-2.5
      ...
      sm:w-auto px-44 py-2.5 mr-7 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Reset</button>
      </div>
    </form>
  </div>
</template>